...
TrenchCoat InMemoryRepo::remove_trench_from_repo(TrenchCoat &t) {
    ...
    TrenchCoat removed_coat = this->trench_coats[position - this->trench_coats.begin()];
    this->trench_coats.erase(position);
    return removed_coat;
}

TrenchCoat InMemoryRepo::update_trench_in_repo(TrenchCoat &t) {
    auto position = std::find(this->trench_coats.begin(), this->trench_coats.end(), t);
    if (position == this->trench_coats.end()) {
        throw RepositoryException("\nError! Cannot update. The trench coat is not in the store.\n");
    }
    TrenchCoat updated_coat = this->trench_coats[position - this->trench_coats.begin()];
