...
void UI::ui_print_all_trench_coats() {
    DynamicVector<TrenchCoat> all_trench_coats = this->srv.get_all_trenches_from_repo();
    if (all_trench_coats.get_dynamic_vector_size() == 0) {
...
void UI::ui_see_filtered_1_by_1() {
    ...
    DynamicVector<TrenchCoat> filtered = this->srv.filter_by_size(user_size);
    if (filtered.get_dynamic_vector_size() == 0) {
    ...
    while (keep_running) {
        ...
        index++;
        if (index == filtered.get_dynamic_vector_size() + 1) index = 1;
...
void UI::ui_print_shopping_cart() {
    DynamicVector<TrenchCoat> shopping_cart_coats = this->srv.get_all_trenches_from_shopping_cart();
    if (shopping_cart_coats.get_dynamic_vector_size() == 0) {
        std::cout << "\nThere are no trench coats in the repository.\n";
        return;
    }
    std::cout << "\nThese are all the trench coats in the repository:\n";
