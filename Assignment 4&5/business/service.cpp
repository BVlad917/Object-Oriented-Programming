...
Service::Service(Repository &repo, RepeatedInstancesRepository &cart)
...
DynamicVector<TrenchCoat> Service::get_all_trenches_from_repo() {
    return this->repo.get_repository_trenches();
}

DynamicVector<TrenchCoat> Service::get_all_trenches_from_shopping_cart() {
    return this->shopping_cart_repo.get_repository_trenches();
}

DynamicVector<TrenchCoat> Service::filter_by_size(std::string size) {
    for (auto &c: size) c = toupper(c);
    DynamicVector<TrenchCoat> all_trenches = this->get_all_trenches_from_repo();
    if (size.length() == 0) return all_trenches;
    DynamicVector<TrenchCoat> filtered;
    for (auto &trench : all_trenches) {
        if (trench.get_trench_size() == size) {
            filtered.add_to_dynamic_vector(trench);
        }
    }
    return filtered;
}
...
double Service::get_total_price() {
    double sum = 0;
    DynamicVector<TrenchCoat> all_trenches_in_cart = this->shopping_cart_repo.get_repository_trenches();
