...
#include "../persistency/file_repository.h"
...
void RepositoryTests::test_get_repository_trenches() {
    ...
    repo.add_trench_to_repo(t3);
    assert(repo.get_repository_size() == 3);
    DynamicVector<TrenchCoat> da = repo.get_repository_trenches();
    assert(da.get_dynamic_vector_size() == 3);
    assert(da[0] == t1);
    assert(da[1] == t2);
    assert(da[2] == t3);
...
void RepositoryTests::test_repeated_instances_repository() {
    RepeatedInstancesRepository repo;
...
void RepositoryTests::test_file_repository_write() {
    FileRepository repo{"test_store_repo.txt"};
...
void RepositoryTests::test_file_repository_add() {
    ...
    FileRepository second_repo{"test_store_repo.txt"};
